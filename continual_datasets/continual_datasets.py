...
class DomainNet2(torch.utils.data.Dataset):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):
        self.root = os.path.expanduser(root)
        self.transform = transform
        self.target_transform=target_transform
...
class DomainNet(torch.utils.data.Dataset):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):
        ...
        if self.train:
            train_data_config = yaml.load(open('splits/domainnet_train.yaml', 'r'), Loader=yaml.Loader)
            self.data = np.array(train_data_config['data'])
            self.targets = np.array(train_data_config['targets'])

        else:
            test_data_config = yaml.load(open('splits/domainnet_test.yaml', 'r'), Loader=yaml.Loader)
    ...
    def process(self,path) :
        return path.replace('data/DomainNet','/data/dataset/liqiwei/domainnet') 

    def __getitem__(self, idx):
        with open(self.process(self.data[idx]), 'rb') as f:
            image = Image.open(f).convert('RGB')
        if self.transform is not None:
            image = self.transform(image)
