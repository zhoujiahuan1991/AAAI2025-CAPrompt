...
from peft.prompt.caprompt import EPrompt
...
class Block(nn.Module):
    ...
    def forward(self, x, prompt=None,adapt=None,task=0,weight=None):
        ...
        if adapt :
            if weight !=None :
                for index in range(task+1) :
                    if index<task :
                        with torch.no_grad():
                            adapt_x = adapt[index](x, add_residual=False).detach().clone() * weight[:,index].unsqueeze(1).unsqueeze(1) + adapt_x
                    else :
                        adapt_x = adapt[index](x, add_residual=False) * weight[:,index].unsqueeze(1).unsqueeze(1) + adapt_x
            else :
                adapt_x = adapt[task](x, add_residual=False)  + adapt_x
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, task_id=-1, prompt_weight=None, train=False):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if self.use_e_prompt:
                ...
                if self.use_e_prompt :
                    ...
                    res = self.e_prompt( prompt_weight=prompt_weight,task_id=task_id)
    ...
    def forward_head(self, res, pre_logits: bool = False):
        ...
        res['logits_detach'] = x @ self.head.weight.detach().clone().T + self.head.bias.detach().clone()
        return res

    def forward(self, x, task_id=-1,  prompt_weight=None, train=False, fc_only=False,):
        if fc_only:
            res = dict()
            x = self.mlp(x)
            x = self.fc_norm(x)
            res['logits'] = self.head(x)
            return res
        res = self.forward_features(x, task_id=task_id, prompt_weight=prompt_weight, train=train)
        res = self.forward_head(res)
        return res

    def linear_constrain(self,task_id,args):
        prompt = self.e_prompt.prompt
...
def _create_vision_transformer(variant, pretrained=False, **kwargs):
    ...
    print('create VisionTransformer')
